...
>
	...
	<div class="mb-4 flex items-center gap-3">
		...
		<div>
			...
			/>
			<div class="text-[10px] uppercase tracking-wider text-gray-500 mt-0.5">Custom JS Code</div>
		</div>
	</div>

	<!-- Configuration -->
	<div class="flex flex-col gap-2 rounded-lg bg-gray-50 border border-gray-100 p-2 nodrag">
		<label class="text-[10px] font-bold uppercase tracking-wider text-gray-500 px-1" for="code-textarea-{id}">JavaScript code</label>
		<textarea
			id="code-textarea-{id}"
			bind:value={data.code}
			class="w-full h-32 bg-white text-gray-800 font-mono text-[11px] p-3 rounded-md border border-gray-200 focus:border-gray-400 focus:ring-1 focus:ring-gray-200 outline-none resize-y shadow-sm"
			spellcheck="false"
		></textarea>
	</div>
