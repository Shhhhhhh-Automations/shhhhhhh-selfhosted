    ...
    <div class="relative z-10">
      {#if step === 1}
        <div class="text-center space-y-6">
          ...
          <p class="text-white/50 text-lg font-light leading-relaxed">Let's get your automation engine ready. Create your Admin account to secure your instance.</p>
          <button 
            onclick={() => step = 2}
      ...
      {:else if step === 2}
        <div class="space-y-8">
          <div class="space-y-2">
            <h2 class="text-3xl font-bold tracking-tight text-white">Create Admin</h2>
            <p class="text-white/40 font-light">Set up your credentials and instance name.</p>
          </div>
          ...
          <form class="space-y-6" onsubmit={(e) => { e.preventDefault(); handleSetup(); }}>
            <div class="space-y-3">
              ...
              <input 
                ...
                required
                class="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-3 text-lg text-white placeholder:text-white/20 focus:border-white/30 focus:bg-white/10 outline-none transition-all duration-300"
                placeholder="e.g. My Automation Lab"
              />
            </div>

            <div class="space-y-3">
              <label for="email" class="block text-sm font-medium text-white/60 ml-1">Admin Email</label>
              <input 
                id="email"
                type="email" 
                bind:value={email}
            ...
            <div class="space-y-3">
              <label for="password" class="block text-sm font-medium text-white/60 ml-1">Admin Password</label>
              <input 
                id="password"
                type="password" 
                bind:value={password}
      ...
      {:else if step === 3}
        <div class="text-center space-y-6">
          ...
          <h1 class="text-4xl font-bold tracking-tight text-white">Instance Secured!</h1>
          <p class="text-white/50 text-lg font-light leading-relaxed">Your <span class="text-white font-medium">{platformName}</span> instance is ready and your session is active.</p>
          <button 
